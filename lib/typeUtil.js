  ...
  default:
    if (Array.isArray(value)) {
      ...
      if (firstItemType === 'string') {
        ...
        var objArray = []
        for (var i = 0; i < value.length; i++) {
          objArray.push(valueToObject(value[i]))
        }
        return {L: objArray}
      } else {
      ...
      for (var property in value) {
        values[property] = valueToObject(value[property])
      }
      return {M: values}    
    } else {
...
function objectToValue(obj) {
  switch (objectToType(obj)) {
    ...
    case 'M':
      var value = {}
      for (var index in obj['M']) {
        value[index] = objectToValue(obj['M'][index])
      }
      return value
    case 'L':
      return obj.L.map(function (o) { return objectToValue(o) })
    default:
